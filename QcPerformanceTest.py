...
image_path = [os.path.join(data_path, truth_table['MainPath'][i], str(truth_table['Folder'][i])).replace("\\",os.sep) \
...
for c in expected_result.columns[:-1]:
    ...
    #print c + '| False positives= ' + str(100*positive_error_rate) + ' % | False negatives=  ' + str(100*negative_error_rate) + ' %'
    print '{:>12} | False positives= {:4.1f} % | False negatives= {:4.1f} %'.format(c, 100*positive_error_rate, 100*negative_error_rate)